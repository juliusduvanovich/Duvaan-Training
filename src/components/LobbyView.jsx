...
// Custom SVG mic icon
function MicIcon({ size=16, color="rgba(201,168,76,0.5)" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
      <rect x="5" y="1" width="6" height="9" rx="3" stroke={color} strokeWidth="1.2"/>
      <path d="M3 8c0 2.76 2.24 5 5 5s5-2.24 5-5" stroke={color} strokeWidth="1.2" strokeLinecap="round"/>
      <line x1="8" y1="13" x2="8" y2="15" stroke={color} strokeWidth="1.2" strokeLinecap="round"/>
...
// Custom SVG pen icon
function PenIcon({ size=16, color="rgba(201,168,76,0.5)" }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
      <path d="M11 2l3 3-8 8H3v-3l8-8z" stroke={color} strokeWidth="1.2" strokeLinejoin="round"/>
      <line x1="9" y1="4" x2="12" y2="7" stroke={color} strokeWidth="1.2"/>
    </svg>
  )
}
...
const css = `
  ...
  @keyframes typingWave {
    0%,60%,100% { opacity:0.15; transform:translateY(0) scale(0.8); }
...
function useSpringTilt() {
  ...
  const onInteract=useCallback((e)=>{
    ...
    elRef.current._rt=setTimeout(()=>{targetX.current=0;targetY.current=0;},300);
  },[]);
...
function ElielGlow({size=260, auraColor='#C9A84C', glowColor='#C9A84C'}){
  ...
  useEffect(()=>{
    ...
    const img=new Image(); img.crossOrigin='anonymous'; img.src='/ElielGold.png';
    const startGlow=(edgePath)=>{
      let sw=false,ss=null,ns=Date.now()+500;
      function draw(){
    ...
    const makeFallbackPath=()=>{const pts=[];const cx=size/2,cy=size/2,r=size*0.44;for(let a=0;a<Math.PI*2;a+=0.015)pts.push({x:cx+Math.cos(a)*r,y:cy+Math.sin(a)*r});return pts;};
...
export default function LobbyView({ onNavigate, settings }) {
  ...
  useEffect(() => {
    const FRAMES = [
      { frame:0, duration: 3200 + Math.random()*1800 },
      { frame:1, duration: 60 },
      { frame:2, duration: 80 },
      { frame:3, duration: 60 },
    ]
    ...
    const next = () => {
      ...
      timer=setTimeout(next, idx===0 ? 3200+Math.random()*1800 : FRAMES[idx].duration)
    }
  ...
  useEffect(()=>{
    ['/ElielGold.png','/ElielGoldHC.png','/ElielGoldFC.png'].forEach(src=>{const img=new Image();img.src=src;})
  },[])
  ...
  useEffect(()=>{
    const iv=setInterval(()=>{ setMsgIndex(i=>{setMsgKey(k=>k+1);return(i+1)%allMsgs.length}) },5000)
  ...
  const startVoice = () => {
    ...
    const rec=new SR(); rec.lang='fi-FI'; rec.continuous=false; rec.interimResults=true
    rec.onstart=()=>setListening(true)
    rec.onresult=e=>{const t=Array.from(e.results).map(r=>r[0].transcript).join('');setTranscript(t);if(e.results[e.results.length-1].isFinal){setTranscript('');setListening(false);if(t.trim())handleSendText(t.trim())}}
  ...
  const handleSendText = async (txt) => {
    const userMsg=(txt||message).trim()
    if(!userMsg||loading) return
    setMessage("")
    const newMessages=[...messages,{role:"user",content:userMsg}]
    setMessages(newMessages); setLoading(true)
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:ELIEL_SYSTEM,messages:newMessages})})
      const data=await res.json()
      setMessages(prev=>[...prev,{role:"assistant",content:data.content?.[0]?.text||"..."}])
    } catch { setMessages(prev=>[...prev,{role:"assistant",content:"Jokin meni pieleen."}]) }
    setLoading(false)
  }

  // Custom gear SVG using aura color
  const GearIcon = ({ color = aura.color }) => (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <circle cx="9" cy="9" r="2.8" stroke={color} strokeWidth="1.3"/>
      {[0,45,90,135,180,225,270,315].map((deg,i)=>{
        const a=deg*Math.PI/180
        const x1=(9+4.0*Math.cos(a)).toFixed(2), y1=(9+4.0*Math.sin(a)).toFixed(2)
        const x2=(9+5.8*Math.cos(a)).toFixed(2), y2=(9+5.8*Math.sin(a)).toFixed(2)
        return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth="1.5" strokeLinecap="round"/>
      })}
      <circle cx="9" cy="9" r="7" stroke={color} strokeWidth="0.8" strokeOpacity="0.3"/>
    </svg>
  )
  ...
  return (
    <>
      <style>{css}</style>
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"35vh 24px 100px", position:"relative" }}>

        {/* ELIEL */}
        <div ref={elRef}
          onClick={(e)=>{onInteract(e);if(!chatOpen)setChatOpen(true)}}
          onTouchStart={(e)=>{onInteract(e);if(!chatOpen)setChatOpen(true)}}
          style={{ cursor:"pointer", willChange:"transform", transformStyle:"preserve-3d", position:"relative", marginBottom: chatOpen?16:24 }}
        >
        ...
        {/* MESSAGE AREA */}
        <div style={{ textAlign:"center", minHeight:80, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", marginBottom:28, padding:"0 8px", width:"100%", maxWidth:320 }}>
          {!chatOpen && (
            <div key={msgKey} className="lobby-msg" style={{ fontFamily:"'Cinzel',serif", fontSize:13, fontWeight:400, letterSpacing:"0.1em", lineHeight:1.7 }}>
              {renderBubbleText()}
            </div>
          )}
        ...
        {chatOpen && (
          <div style={{ width:"100%", maxWidth:320, display:"flex", flexDirection:"column", alignItems:"center" }}>
              ...
              <div style={{ width:"100%", textAlign:"center" }}>
                {listening ? (
                  <>
                    ...
                    <button onClick={stopVoice} style={{ background:"none", border:`1px solid rgba(201,168,76,0.2)`, borderRadius:20, padding:"6px 20px", cursor:"pointer", color:"rgba(201,168,76,0.5)", fontFamily:"'Cinzel',serif", fontSize:9, letterSpacing:"0.16em", textTransform:"uppercase" }}>Lopeta</button>
                  </>
                ) : (
                  <button onClick={startVoice} style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(201,168,76,0.4)", fontFamily:"'Cinzel',serif", fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase", display:'flex', alignItems:'center', gap:8 }}>
                    <MicIcon color="rgba(201,168,76,0.5)"/> Puhu
                  </button>
                )}
              </div>
            )}
            ...
            <div style={{ display:"flex", gap:20, marginTop:12, alignItems:"center" }}>
              <button onClick={()=>{ const next=inputMode==='text'?'voice':'text'; setInputMode(next); if(next==='voice') startVoice(); else stopVoice() }}
                style={{ background:"none", border:"none", cursor:"pointer", padding:0, display:'flex', alignItems:'center' }}>
                {inputMode==='text' ? <MicIcon color="rgba(201,168,76,0.3)"/> : <PenIcon color="rgba(201,168,76,0.3)"/>}
              </button>
              <button onClick={()=>{setChatOpen(false);setMessages([]);setMessage("");stopVoice()}}
                style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(201,168,76,0.18)", fontFamily:"'Cinzel',serif", fontSize:9, letterSpacing:"0.18em", textTransform:"uppercase", padding:0 }}>×</button>
            </div>
          </div>
        )}
      </div>
